...
//   }
// };
// orderController.js - UPDATED TO MATCH DASHBOARD DATA
...
async function sendBulkNotifications(userIds, message, type, data, options) {
  // Implement bulk notification logic here
  console.log("Bulk notification would be sent to:", userIds.length, "users");
}
...
export const getAllOrders = async (req, res) => {
  try {
    ...
    // Get total count for pagination (without search filter for total count)
    const totalCountQuery = status && status !== "all" ? { status } : {};
    const total = await Order.countDocuments(totalCountQuery);
    ...
    // If search term provided, filter after populating
    let filteredOrders = orders;
    if (search && orders.length > 0) {
      const searchLower = search.toLowerCase();
      filteredOrders = orders.filter((order) => {
    ...
    // Get status counts (for the filtered query)
    const statusCounts = {};
    const statuses = [
      "pending",
      "processing",
    ...
    // Calculate total revenue (only from delivered orders) - MATCH DASHBOARD
    ...
    // Calculate average order value (from all orders) - MATCH DASHBOARD
    ...
    const averageOrderValue =
      allOrders.length > 0 ? totalOrderValue / allOrders.length : 0;
    ...
    res.json({
      ...
      data: filteredOrders,
      pagination: {
        total: total, // Use total count without search for pagination
        page: parseInt(page),
        limit: parseInt(limit),
...
// UPDATED getOrderStats TO MATCH DASHBOARD
// orderController.js - Updated getOrderStats function

export const getOrderStats = async (req, res) => {
  try {
    console.log("📈 Fetching order stats (matching dashboard)...");

    // Get total counts - SAME AS DASHBOARD
    const totalOrders = await Order.countDocuments();

    // Order status counts - MUST match dashboard exactly
    ...
    // Revenue calculation (only from delivered orders) - SAME AS DASHBOARD
    const deliveredOrders = await Order.find({ status: "delivered" });
    const totalRevenue = deliveredOrders.reduce(
      (sum, order) => sum + (order.total || 0),
      0
    ...
    // Average order value (from all orders) - SAME AS DASHBOARD
    const allOrders = await Order.find();
    const totalOrderValue = allOrders.reduce(
      (sum, order) => sum + (order.total || 0),
      0
    ...
    res.json({
      success: true,
      data: {
        // Core stats - MUST match dashboard
        totalOrders,
        pending,
        processing,
        shipped,
        ...
        // For frontend convenience - EXACTLY match dashboard structure
        statusCounts: {
          pending,
          processing,
          shipped,
    ...
    console.log("✅ Order stats fetched successfully (matching dashboard)");
  } catch (err) {
