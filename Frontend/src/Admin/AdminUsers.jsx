...
const AdminUsers = () => {
  ...
  const loadUsers = async () => {
    try {
      ...
      const res = await fetchWithAuth(
        `${BACKEND_URL}/api/admin/users?${params}`
      );
  ...
  const toggleBlock = async (id, isBlocked) => {
    ...
    try {
      ...
      const res = await fetchWithAuth(
        `${BACKEND_URL}/api/admin/users/${id}/${endpoint}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const toggleRole = async (id, currentRole) => {
    if (
      !window.confirm(
        `Are you sure you want to ${currentRole === "admin" ? "remove admin role from" : "make admin"} this user?`
      )
    ) {
      return;
    }
  ...
  const forceLogout = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(result.message || "Force logout failed");
      }
    } catch (err) {
      console.error("Error forcing logout:", err);
  ...
  if (error) {
    return (
      <div
        className={`min-h-screen flex items-center justify-center ${themeClasses.container}`}
      >
