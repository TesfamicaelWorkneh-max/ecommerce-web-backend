...
const ProductCard = ({ product, index = 0 }) => {
  ...
  const discountPercent = useMemo(
    () =>
      isOnSale
        ...
        : 0,
    [isOnSale, prod.originalPrice, prod.price]
  );
  ...
  const handleProductClick = useCallback(
    (e) => {
      // Prevent default only for quick view button
      if (e.target.closest("button[data-quickview]")) {
        e.preventDefault();
        ...
        setTimeout(() => {
          navigate(`/product/${prod._id}`, {
            state: {
              product: prod,
              scroll: false, // Prevent scroll restoration issues
            },
          });
        }, 50);
      }
    ...
    <div className="group relative overflow-hidden">
      ...
      <div className="relative rounded-2xl overflow-hidden">
        <Link
          ...
          state={{ product: prod, scroll: false }}
        >
          <div className="relative w-full h-60">
            ...
            <img
              ...
              fetchpriority={index < 3 ? "high" : "auto"} // High priority for first 3 images
            />
          </div>
        </Link>
        ...
        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
          <button
            data-quickview
            onClick={handleProductClick}
            className="px-4 py-2 bg-primaryBg dark:bg-gray-800 text-dark dark:text-white rounded-lg font-medium flex items-center gap-2 hover:scale-105 transition-transform duration-200"
          >
            <FaEye />
            Quick View
          </button>
      ...
      <div className="p-4">
        ...
        <div className="mb-3">
          {prod.isSold ? (
            ...
            <button
              className="w-full px-2 py-3 bg-accent dark:bg-accent/80 hover:bg-lightBg dark:hover:bg-lightBg/30 text-dark dark:text-white rounded-lg font-semibold flex items-center justify-center gap-2 transition-colors duration-300 disabled:opacity-70"
              onClick={addToCart}
              disabled={loadingAdd}
            >
              {loadingAdd ? (
