...
//         }
//       } else {
//         // Navigate to MainPage first, then scroll after load
...
const Footer = () => {
  const [loading, setLoading] = useState(true);
  const [snowCount] = useState(50); // optional snowflakes
  const controls = useAnimation();
  const ref = React.useRef();
  const isInView = useInView(ref, { once: true, amount: 0.05 });
  const navigate = useNavigate();
  ...
  const trustBadges = [
    ...
    { icon: <FaCreditCard />, text: "Easy Returns" },
  ...
  return (
    <>
      ...
      <motion.footer
        ...
        animate={controls}
        variants={containerVariants}
        className="relative z-10 pt-12 pb-8 w-full px-4 sm:px-6 bg-white dark:bg-slate-900 transition-colors duration-500"
      >
        <div className="max-w-7xl mx-auto w-full">
          {/* Top Section */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 pb-8 sm:pb-12 border-b border-gray-300 dark:border-slate-700">
            {/* Logo & Description */}
            <motion.div
              variants={slideInLeft}
              className="space-y-4 sm:space-y-6 sm:col-span-2 lg:col-span-1"
            >
              <div className="flex items-center gap-3">
                <motion.div
                  ...
                  className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center bg-gradient-to-r from-amber-400 to-rose-400 dark:from-amber-500 dark:to-rose-500"
                >
                  <IoSparkles className="text-lg sm:text-xl text-white" />
                </motion.div>
                <div>
              ...
              <p className="text-sm sm:text-base text-gray-700 dark:text-slate-400">
                Your trusted destination for premium products, quality, and
                outstanding service since 2025.
              </p>
              <div className="flex gap-3 sm:gap-4">
                {socialLinks.map((social, index) => (
                  <motion.a
                    ...
                    transition={{ delay: index * 0.07 }}
                    whileHover={{ scale: 1.1, rotate: 5 }}
                    className="w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center bg-gray-100 dark:bg-slate-700 text-gray-800 dark:text-white"
                  >
                    {social.icon}
                  </motion.a>
                ))}
              ...
              <ul className="space-y-2 sm:space-y-3">
                {quickLinks.map((link, index) => (
                  <motion.li key={index} variants={itemVariants}>
                    <button
                      onClick={() => handleFooterLinkClick(link)}
                      className="text-gray-700 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white transition-colors duration-300"
                    >
                      {link.name}
                    </button>
                  </motion.li>
              ...
              <div className="space-y-4">
                {contactItems.map((item, index) => (
                  <motion.div
                    ...
                    className="flex items-start gap-3"
                  >
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center bg-gray-100 dark:bg-slate-700 text-gray-800 dark:text-white">
                      {item.icon}
                    </div>
                    <div>
                      ...
                      <div className="text-xs sm:text-sm text-gray-700 dark:text-slate-400 break-words">
                        {item.content}
                      </div>
                    </div>
                  </motion.div>
          ...
          <motion.div
            variants={fadeInUp}
            className="pt-6 sm:pt-8 border-t border-gray-300 dark:border-slate-700"
          >
            <div className="flex flex-col gap-4 sm:gap-6 text-center text-gray-700 dark:text-slate-400 text-xs sm:text-sm">
              <span>© 2025 Ades. All rights reserved.</span>
              <span>
                Made with <FaHeart className="inline text-red-500" /> by the{" "}
                <strong>Ades</strong> team
              </span>
            </div>
          </motion.div>
        </div>
